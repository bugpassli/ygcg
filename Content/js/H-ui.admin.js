...
function user_start(obj,id){
	layer.confirm('确认要查看吗？',function(index){
		$(obj).parents("tr").find(".user-manage").prepend('<a style="text-decoration:none" onClick="user_stop(this,\'10001\')" href="javascript:;" title="查看"><i class="icon-hand-down"></i></a>');
		$(obj).parents("tr").find(".user-status").html('<span class="label label-success">查看</span>');
		$(obj).remove();
		layer.msg('已启用!',1);
	});
}
...
function user_del(obj,id){ 
	layer.confirm('确认要删除吗？',function(index){		
		$.ajax
	    ({ 
		     url: "/Online_E_System/deleteUser.action", //登录处理页
		     dataType: "text",
		     //传送请求数据
		     data: { id: id },
...
function datadel(){
   ...
   var ids = new Array();  
   var obj=$('input[name=user-ids]:checked');
    $('input[name=user-ids]:checked').each(function(){  
    ...
    if(ids.length>0){
	    ...
	    ({ 
	    	 type:"post",
		     url: "/Online_E_System/deletetUsers.action", //登录处理页
		     dataType: "text",
		     //传送请求数据
		     data:"ids="+ JSON.stringify(ids),// 将数组序列化
		     success: function (strValue) { //登录成功后返回的数据
		      //根据返回值进行状态显示
		    	 if(strValue=="ok"){		    		 
		    		 $(obj).parents("tr").remove();
...
function subject_del(obj,id){ 
	layer.confirm('确认要删除吗？',function(index){		
		$.ajax
	    ({ 
		     url: "/Online_E_System/deleteSubject.action", //登录处理页
		     dataType: "text",
		     //传送请求数据
		     data: { id: id },
		     success: function (strValue) { //登录成功后返回的数据
...
function subjectDel(){
   ...
   var ids = new Array();  
   var obj=$('input[name=subject-ids]:checked');
    $('input[name=subject-ids]:checked').each(function(){  
    ...
    if(ids.length>0){
	    ...
	    ({ 
	    	 type:"post",
		     url: "/Online_E_System/deletetSubjects.action", //登录处理页
		     dataType: "text",
		     //传送请求数据
		     data:"ids="+ JSON.stringify(ids),// 将数组序列化
		     success: function (strValue) { //登录成功后返回的数据
...
function student_del(obj,id){ 
	layer.confirm('确认要删除吗？',function(index){		
		$.ajax
	    ({ 
		     url: "/Online_E_System/deleteStudent.action", //登录处理页
		     dataType: "text",
		     //传送请求数据
		     data: { id: id },
		     success: function (strValue) { //登录成功后返回的数据
...
function studentdel(){
   ...
   var ids = new Array();  
   var obj=$('input[name=student-ids]:checked');
    $('input[name=student-ids]:checked').each(function(){  
    ...
    if(ids.length>0){
	    ...
	    ({ 
	    	 type:"post",
		     url: "/Online_E_System/deleteStudents.action", //登录处理页
		     dataType: "text",
		     //传送请求数据
		     data:"ids="+ JSON.stringify(ids),// 将数组序列化
		     success: function (strValue) { //登录成功后返回的数据
...
function classes_del(obj,id){ 
	layer.confirm('确认要删除吗？',function(index){		
		$.ajax
	    ({ 
		     url: "/Online_E_System/deleteClasses.action", //登录处理页
		     dataType: "text",
		     //传送请求数据
		     data: { id: id },
		     success: function (strValue) { //登录成功后返回的数据
...
/*------------知识点管理--------------*/
function points_add(w,h,title,url){
	layer_show(w,h,title,url);
}

function points_show(w,h,title,url){
	layer_show(w,h,title,url);
}

function points_edit(w,h,title,url){
	layer_show(w,h,title,url);
}
/*------------资讯管理----------------*/
/*获取分类值*/
